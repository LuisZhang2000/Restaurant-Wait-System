...
import { Button, Container, Nav, Navbar as NavbarBs, InputGroup, Modal } from "react-bootstrap"
import { raiseAssistance, getAssistanceRaised } from '../endpoints/table';

export function Navbar({ tableNumber, setTableNumber, cartItemsNumber, disableTableEdit }) {
    ...
    return (
        <NavbarBs sticky="top" className="bg-white shadow-sm">
            <Container>
                ...
                <div className="mx-4 d-flex justify-content-between" style={{ 'width': '380px' }}>
                    <Button
                    ...
                    >
                        {tableNumber === null ? 'Enter Table Number' : `Table ${tableNumber}`}
                    </Button>
                    <Button
                        variant="primary"
                        disabled={disableAssistance}
                        onClick={() => {
                            raiseAssistance(tableNumber)
                            .then(() => {
